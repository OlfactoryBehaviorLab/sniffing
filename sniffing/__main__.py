...
def select_dialog(file = False):
    dialog=QFileDialog()
    if file:
        _val, _ = dialog.getOpenFileName(None, 'Select H5 File: ', filter="H5 Files (*.h5)")
    else:
...
def main():
    ...
    parser.add_argument('-s', '--single', help="Path to single file to process", nargs='?', const='', default=None)
    parser.add_argument('-c', '--combined', action='store_true', help="Run combined analysis")
    parser.add_argument('-b', '--batch', action='store_true', help='Batch process H5 files')
    parser.add_argument('-d', '--data_dir', help="Path to data directory, implies --batch")
    ...
    # The default is batch processing, so see if the user explicitly specified, or didn't specify a single file
        ...
        if args.data_dir:
            ...
            if len(_path) > 0:
                ...
                if not _path.exists():
                    raise FileNotFoundError(f'Default Directory {DEFAULT_DIR} not found!')
                data_dir = _path
