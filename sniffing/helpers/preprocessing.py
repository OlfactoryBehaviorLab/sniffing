...
def get_trace_features(trace: pd.Series) -> tuple[pd.Series, pd.Series, np.array]:
    ...
    exhalation_peak, _ = signal.find_peaks(
        trace * -1, distance=30, width=10, prominence=0.1
    )
    inhale_peak, _ = signal.find_peaks(trace, distance=30, width=10, prominence=0.1)
...
def filter_sniff_peaks(inhales: pd.Series, exhales: pd.Series):
    ...
    while True:
        ...
        if inhale_index >= inhales.shape[0] - 1:  # Do while loop proxy
            ...
            if run_start == 0:
                good_inhales.loc[current_inhale, "magnitude"] = inhales.index[
                    inhale_index
                ]
            else:
                true_sniff_timestamp, true_sniff_magnitude = _pick_true_peaks(
                    inhales, run_start, inhale_index
                )
                good_inhales.loc[true_sniff_timestamp, "magnitude"] = (
                    true_sniff_magnitude
                )
...
def get_inhalation_durations(
    true_inhales: pd.DataFrame, flanking_exhales: pd.DataFrame, sniff_trace
):
    inhalation_durations = pd.DataFrame(
        index=true_inhales.index, columns=["duration", "left_ts", "right_ts"]
    )
